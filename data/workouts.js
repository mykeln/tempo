{
    "config": [
        {
            "name": "Mykel Nahorniak",
            "weight": "145"
        }
    ],
    "fitness": [
        {
            "1s": "1705",
            "5s": "1393",
            "30s": "971",
            "1m": "610",
            "5m": "406",
            "10m": "324",
            "20m": "310",
            "1h": "299"
        }
    ],
    "activities": [
        {
            "name": "Stretching",
            "shortname": "stretch",
            "type": "stretch",
            "wu": "Do 10 slow body weight squats.",
            "desc": "Stretch various parts of your legs and back.",
            "info": "",
            "target": "",
            "cd": "",
			"duration": "30"
        },
        {
            "name": "Run - Easy Run",
            "shortname": "run-easy",
            "type": "run",
            "wu": "1 minute of light jogging",
            "desc": "Run at a steady pace for around 20 minutes.",
            "info": "Just to keep the legs loose. No hard efforts.",
            "target": "",
			"cd": "",
			"duration": "20"
        },
        {
            "name": "AC - 1K Time Trial",
            "shortname": "ac-1ktimetrial",
            "type": "bike",
            "wu": "5 - 8 hard efforts at and above threshold lasting 1 - 5 minutes each",
            "desc": "These are full gas 1K efforts. Time will depend on speed. Stay aero and keep the cadence high (105+). Recover with some easy spinning between efforts. Complete 5 - 10 intervals. Power is anaerobic capacity for each.",
            "info": "",
            "target": "([1m]*1) watts",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "AC - 1 - 2 minutes (hills)",
            "shortname": "ac-1-2minuteshills",
            "type": "bike",
            "wu": "3 - 5 hard efforts at and above threshold lasting 1 - 5 minutes each.",
            "desc": "This is a basic hill intervals ride. Find a hill that you can do over and over again or choose a course with a bunch of hills that are 3 - 5 minutes apart. Each hill should take 1 - 2 minutes each going as hard as you can. EXPLODE at the top of each hill. Recover with some easy spinning between efforts. Complete 8 - 10 intervals.",
            "info": "The goal is to carry the power for as long as possible on each climb. Stand for the last 10 - 15 seconds if you need to.",
            "target": "([1m]*.9) watts",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "AC - 30 - 60 seconds",
            "shortname": "ac-30-60seconds",
            "type": "bike",
            "wu": "5 x 1 minute of hard pedaling and 1 x 5 minutes in the lower half of your threshold zone.",
            "desc": "Start with 3 x 30 seconds with 5 minutes between intervals. Spin easy for 5 minutes. Complete 3 x 45 seconds with 5 minutes between intervals. Spin easy for 5 minutes. Complete 3 x 60 seconds with 5 minutes between intervals.",
            "info": "Target power is your best 45 seconds. You can do these on the trainer. Just wind into them. Really need to stay with the target power for all of these. We want to stretch the capacity which not only doing lots hard but doing them longer. Don't do the 30s too hard otherwise you will not be able to do the 60s.",
            "target": "([1m]*0.96322241681261) watts",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "AC - 30 seconds",
            "shortname": "ac-30seconds",
            "type": "bike",
            "wu": "5 x 1 minute of hard pedaling and 1 x 5 minutes in the lower half of your threshold zone.",
            "desc": "Start with 3 x 30 seconds with 5 minutes between intervals stepping your way up to the target power. Complete up to 10 more intervals of 30 seconds each at the target power with 5 minutes of recovery between effort. Stop when you cannot hold the target power for 20 seconds.",
            "info": "Target power is your best 45 seconds, in steps. Stop if you cannot stay over the target for at least 20 seconds. Recovery should be pretty complete between efforts.",
            "target": "First: ([1m]*0.788091068301226) watts. Second: ([1m]*0.875656742556918) watts. Third: ([1m]*0.945709281961471) watts. The rest: ([1m]*0.96322241681261) watts",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "AC - 3x30 seconds",
            "shortname": "ac-3x30seconds",
            "type": "bike",
            "wu": "5 x 1 minute of hard pedaling and 1 x 5 minutes in the lower half of your threshold zone.",
            "desc": "Start with 3 x 30 seconds with 5 minutes between intervals stepping your way up to the target power. Complete 15 - 20 repeats of 30 seconds at target power and then 30 seconds of easy spinning. Stop when you cannot hold the target power for at least 20 seconds.",
            "info": "",
            "target": "First 3: ([1m]*0.788091068301226) watts, ([1m]*0.875656742556918) watts, ([1m]*0.945709281961471) watts. Another 15 at: ([1m]*0.96322241681261) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "AC - 45 seconds",
            "shortname": "ac-45seconds",
            "type": "bike",
            "wu": "5 x 1 minute of hard pedaling and 1 x 5 minutes in the lower half of your threshold zone.",
            "desc": "Start with 3 x 45 seconds with 5 minutes between intervals stepping your way up to the target power. Complete up to 10 more intervals of 45 seconds each at the target power with 5 minutes of recovery between effort. Stop when you cannot hold the target power for 20 seconds.",
            "info": "Step up over the first 3 and then see if you can hold the rest at target.",
            "target": "First 3: ([1m]*0.700525394045534) watts, ([1m]*0.74430823117338) watts, ([1m]*0.788091068301226) watts. Another 10 at: ([1m]*0.875656742556918) watts",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "AC - Power Wind Ups",
            "shortname": "ac-powerwindups",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Include 10 - 15 Power Wind Ups during the ride. Power Wind Up: Start at 20 mph and increase power and speed steadily for 10-20 seconds, or until you stop accelerating. Hold this speed for 30-60 seconds. Recover completely between efforts.",
            "info": "",
            "target": "([1m]*0.875656742556918) watts",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "AC - Race Winners",
            "shortname": "ac-racewinners",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Include 5 - 7 Race Winners where you wind the speed up with a 15 second hard sprint followed by 60 - 90 seconds of all-out riding as if you were attacking for the finish line. Finish each effort with a 10 second all-out sprint. Recover completely between efforts.",
            "info": "",
            "target": "Each Race Winner should be more by feel, rather than targeting a specific power. Go as hard as you can for the duration.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Endurance - Basic",
            "shortname": "endurance-basic",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Ride steadily in your endurance power zone. Maintain a solid and steady pedal stroke, with cadence in the 90 - 110 range.",
            "info": "",
            "target": "([1h]*0.714285714285714) watts or under.",
			"cd": "Easy spinning for 15 minutes.",
			"duration": "120"
        },
        {
            "name": "Endurance - Basic Group",
            "shortname": "endurance-basicgroup",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "If the group wants to go harder, then sit in and try to keep your power at an endurance pace.",
            "info": "",
            "target": "([1h]*0.714285714285714) watts or under.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "120"
        },
        {
            "name": "Big Block - Attacks",
            "shortname": "bigblock-attacks",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "This is a long day on rolling roads with constant power at least in your endurance zone. In the last 90 minutes include 5 - 7 attacks where you wind the speed up with a 15 second hard sprint followed by 60 - 90 seconds of all-out riding as if you were attacking for the finish line. Finish the attack with a big 10 second sprint. Recover completely between efforts.",
            "info": "",
            "target": "([1h]*0.714285714285714) watts for your endurance zone. The attacking parts should be more by feel, rather than targeting a specific power. Go as hard as you can for the duration.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "210"
        },
        {
            "name": "Big Block - Sprints",
            "shortname": "bigblock-sprints",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "This is a long day on rolling roads with constant power at least in your endurance zone. In the last 90 minutes include 5 - 10 sprints that are 10 - 30 seconds long each. Sprints should be from race speed and full gas.",
            "info": "",
            "target": "([1h]*0.714285714285714) watts for your endurance zone. The sprinting parts should be more by feel, rather than targeting a specific power. Go as hard as you can for the duration.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "210"
        },
        {
            "name": "Big Block - VO2",
            "shortname": "bigblock-vo2",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "This is a long day on rolling roads with constant power at least in your endurance zone. In the last 90 minutes find a hill or hills that takes 2 - 4 minutes to climb at a steady but hard pace. Recover for the same amount of time as the preceding interval. Complete 7 - 10 intervals. Once you are unable to maintain a hard pace for at least 2 minutes, then it is time to head home.",
            "info": "",
            "target": "([1h]*0.714285714285714) watts for your endurance zones. The climbing parts should be more by feel, rather than targeting a specific power. Go as hard as you can.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "210"
        },
        {
            "name": "Bursts - Basic Bursts",
            "shortname": "bursts-basicbursts",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Every 10 minutes, include an 8 second all-out burst. Jump hard with high cadence. Only 8 seconds.",
            "info": "",
            "target": "No power target here; these should be all-out, out of the saddle sprints.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Bursts - Microbursts",
            "shortname": "bursts-microbursts",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Include 3 x 10 minutes of Microbursts. Each burst is 15 seconds long. Start each burst with a hard pop, then settle into a rhythm. Rest for 15 seconds between bursts. Ride in your endurance zone for 5 minutes between sets.",
            "info": "",
            "target": "([1h]*3) watt burst, then settle into a rhythm, averaging ([1h]*1.5) watts over the 15 seconds. Rest at ([1h]*0.5) watts between bursts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Bursts - FTP 2x20 Minutes with Bursts",
            "shortname": "bursts-2x20minutes",
            "type": "bike",
            "wu": "20 - 30 minutes including 1 x 5 minutes of hard pedaling and 10 x 1 minute of high cadence.",
            "desc": "Complete 2 x 20 minutes in the lower half of your threshold power zone. Every 10 minutes, include an 8 second all-out burst. Jump hard with high cadence. Only 8 seconds. Recover between intervals by riding for 10 minutes at or just below your balance point wattage.",
            "info": "",
            "target": "([1h]*.9) watts for your threshold effort. The burst should be by feel, an all-out effort. Balance point wattage: ([1h]*.8)",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "120"
        },
        {
            "name": "Endurance - Basic + Attacks",
            "shortname": "endurance-basic+attacks",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Include 5 - 7 attacks where you wind the speed up with a 15 second hard sprint followed by 60 - 90 seconds of all-out riding as if you were attacking for the finish line. Finish the attack with a big 10 second sprint. Recover completely between efforts.",
            "info": "",
            "target": "Each effort should be more by feel, rather than targeting a specific power. Go as hard as you can for the duration.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "120"
        },
        {
            "name": "Endurance - Group Ride",
            "shortname": "endurance-groupride",
            "type": "bike",
            "wu": "",
            "desc": "Solid ride time including some hammering at the front. Also try for 8 sprints today at least 200m long. Let's see what you got with some kilojoules in your legs. Rest plenty between each. Let the group determine the hammer part. Try to finish with a solid 20 minutes of tempo in at the end of the ride.",
            "info": "",
            "target": "([1h]*0.80) watts for the finishing tempo effort. Let the group determine the rest.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "210"
        },
        {
            "name": "FTP - 2x10 minutes",
            "shortname": "ftp-2x10minutes",
            "type": "bike",
            "wu": "20 - 30 minutes including 1 x 5 minutes of hard pedaling and 10 x 1 minute of high cadence.",
            "desc": "Complete 2 x 10 minutes in the lower half of your threhold power zone. Recover between intervals by riding for 5 minutes at or just below your balance point wattage.",
            "info": "",
            "target": "([1h]*.9) watts for the finishing threshold effort. Balance point wattage: ([1h]*.8) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "FTP - 2x20 minutes",
            "shortname": "ftp-2x20minutes",
            "type": "bike",
            "wu": "20 - 30 minutes including 1 x 5 minutes of hard pedaling and 10 x 1 minute of high cadence.",
            "desc": "Complete 2 x 20 minutes in the lower half of your threhold power zone. In each interval, alternate 2 minutes in the lower half of your threshold zone with 2 minutes in the upper half of your threshold zone. Recover between intervals by riding for 10 minutes at or just below your balance point wattage.",
            "info": "",
            "target": "([1h]*.9) watts - ([1h]*.95) watts for the threshold effort. Balance point wattage: ([1h]*.8) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "FTP - 3x10 minutes",
            "shortname": "ftp-3x10minutes",
            "type": "bike",
            "wu": "20 - 30 minutes including 1 x 5 minutes of hard pedaling and 10 x 1 minute of high cadence.",
            "desc": "Complete 3 x 10 minutes in the lower half of your threhold power zone. Recover between intervals by riding for 5 minutes at 50% of the power of the preceding effort.",
            "info": "",
            "target": "([1h]*.92) watts for the threshold effort. Recover between sets at ([1h]*.46) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "FTP - 3x15 minutes",
            "shortname": "ftp-3x15minutes",
            "type": "bike",
            "wu": "20 - 30 minutes including 1 x 5 minutes of hard pedaling and 10 x 1 minute of high cadence.",
            "desc": "Complete 3 x 15 minutes in the lower half of your threhold power zone. Recover between intervals by riding for 5 minutes at 50% of the power of the preceding effort.",
            "info": "",
            "target": "([1h]*.92) watts for the threshold effort. Recover between sets at ([1h]*.46) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "FTP - 3x20 minutes",
            "shortname": "ftp-3x20minutes",
            "type": "bike",
            "wu": "20 - 30 minutes including 1 x 5 minutes of hard pedaling and 10 x 1 minute of high cadence.",
            "desc": "Complete 3 x 20 minutes in the lower half of your threshold power zone. During the first interval, increase the level of effort by 50% for 10 seconds every 2 minutes. Recover between intervals by riding for 20 minutes at 50% of the power of the preceding effort.",
            "info": "",
            "target": "([1h]*.92) watts for the threshold effort. Recover between sets at ([1h]*.46) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "FTP - 5x3 minutes",
            "shortname": "ftp-5x3minutes",
            "type": "bike",
            "wu": "20 - 30 minutes including 1 x 5 minutes of hard pedaling and 10 x 1 minute of high cadence.",
            "desc": "Complete 3 sets of 5 x 3 minutes your lower threshold power zones. Between each interval, ride for 2 minutes at in the middle of your endurance zone. Between sets, ride for 5 minutes in your endurance zone.",
            "info": "",
            "target": "([1h]*.92) watts 3 minute target. 2 minute target: ([1h]*.56). 5 minutes between sets: ([1h]*0.714285714285714) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Easy or Off",
            "shortname": "easy-off",
            "type": "bike",
            "wu": "",
            "desc": "Chill out, dude.",
            "info": "",
            "target": "",
			"cd": "",
			"duration": "90"
        },
        {
            "name": "Peak - VO2 + AC Step-Ups",
            "shortname": "peak-vo2acstepups",
            "type": "bike",
            "wu": "5 x 1 minute of high cadence pedaling and 1 x 5 minutes of high threshold effort, then easy spinning for 5 minutes.",
            "desc": "Complete the following sequence aiming to average in your VO2 max zone for each.<br/>Set 1: 3 x 3 minutes in upper threshold with 1 minute between intervals in your endurance zone. 5 minutes endurance.<br/>Set 2: 4 x 2 minutes in your upper threshold/VO2max zone with 1 minute between intervals in your endurance zone. 5 minutes endurance.<br/>Set 3: 5 x 1 minute in your AC zone with 1 minute between intervals in your endurance zone. 5 minutes endurance. Set 4: 10 x 40/30s. Do what you can. 40 seconds at VO2, 30 seconds at 70%.",
            "info": "",
            "target": "Set 1: ([1h]*.92) watts. Set 2: ([5m]*1) watts. Set 3: ([1m]*1) watts. Set 4: 40 seconds at ([5m]*1) watts, 30 seconds at ([5m]*0.70) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Race Prep",
            "shortname": "raceprep",
            "type": "bike",
            "wu": "",
            "desc": "Ride 60 - 90 minutes mostly in your endurance zone. Complete 5 low gear accelerations with smooth cadence above 120 rpm lasting about 1 minute each with at least 2 minutes of recovery between efforts. Complete 5 big gear intervals that last no more than 1 minute each. In each interval your goal is to go fast, but back down before you start really killing yourself. Recover for at least 2 minutes between efforts. Include at least 5 out of the saddle jumps that last no longer than 12 seconds each. You can do these are part of the low gear accelerations or big gear intervals.",
            "info": "",
            "target": "Each effort should be more by feel, rather than targeting a specific power. Back down before you start killing yourself to save legs for tomorrow.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Race Prep TT",
            "shortname": "racepreptt",
            "type": "bike",
            "wu": "",
            "desc": "During a 60 - 90 minute ride, do 5 x 2 minutes on your TT bike. Each effort should include a steady acceleration from a slow speed. Get the bike up to TT speed and hold for 60 - 90 seconds. Spin easy at a high cadence between efforts and during the rest of the ride. Stay on the TT bars as much as possible during the ride and always during the efforts.",
            "info": "",
            "target": "Each effort should be more by feel, rather than targeting a specific power. Back down before you start killing yourself to save legs for tomorrow.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Race",
            "shortname": "race",
            "type": "bike",
            "wu": "Spin into endurance for 10-15 mins. Big ring 18. 5-min step up to FTP. Big ring/18-17-16 cogs. Then easy for 5. Then a 3-min step up to VO2 and over. Big ring/16-15-14 cogs. 1 easy-ish out of the saddle jump.",
            "desc": "Race.",
            "info": "",
            "target": "For the warm up, spin up to ([1h]*0.714285714285714) watts in the first 10-15 minutes. Then step up to ([1h]*1) over the next 5 minutes. For the VO2, step up to ([5m]*1) over the next 3 minutes.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Sprints - 6",
            "shortname": "sprints-6",
            "type": "bike",
            "wu": "5 x 1 minute of hard pedaling and 1 x 5 minutes in the lower half of your threshold zone.",
            "desc": "Do 6 sprints. all-out of the saddle the whole way. First 2: Small ring, cog 16 to 15. Start from 17mph. 100m. Second 2: Big ring, cog 17 to 16 to 15. Start from 23mph. 150-200m. Third 2: Big ring, cog 15 to 14. Start from 25mph. 150-200m.",
            "info": "",
            "target": "",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Sprints - 9",
            "shortname": "sprints-9",
            "type": "bike",
            "wu": "5 x 1 minute of hard pedaling and 1 x 5 minutes in the lower half of your threshold zone.",
            "desc": "Do 9 sprints. all-out of the saddle the whole way. First 3: Small ring, cog 16 to 15. Start from 17mph. 100m. Second 3: Big ring, cog 17 to 16 to 15. Start from 23mph. 150-200m. Third 3: Big ring, cog 15 to 14. Start from 25mph. 150-200m.",
            "info": "",
            "target": "",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Sprints - Sprint Starts",
            "shortname": "sprints-sprintstarts",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride.",
            "desc": "Complete 5 sets of 5 Sprint Starts. Each set is 5 minutes long. At the start of each minute and without shifting gears, complete a short super hard, super smooth, out of the saddle jump that is only 6 - 8 strokes. You are going for max power on each effort. Spin at 100 rpm in your endurance zone for the remainder of the minute. Complete 5 starts in each set. Spin easy for 5 minutes between sets.",
            "info": "Works best on the road. You can do these on the trainer if have to. If on the trainer, do the first 3 sets seated.",
            "target": "Endurance wattage: ([1h]*0.7) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Sprints - Stomp Sprints (seated)",
            "shortname": "sprints-stompsprintsseated",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride.",
            "desc": "During the ride, include 6 - 10 stomps where you let the speed drop to near nothing while on your 53x11. While still seated, accelerate hard for 12 - 15 seconds. After the effort, ride in your endurance zone for 5 - 10 minutes between intervals at a cadence of 105+.",
            "info": "",
            "target": "Endurance wattage: ([1h]*0.7) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Sprints - Stomp Sprints (standing)",
            "shortname": "sprints-stompsprintsstanding",
            "type": "bike",
            "wu": "5 x 1 minute of hard pedaling and 1 x 5 minutes in the lower half of your threshold zone.",
            "desc": "Do 11 sprints. Each sprint starts from about 15 mph. Do the sprints in the following sequence. 53x17 - seated<br/>53x16 - seated<br/>53x15 - seated<br/>53x14 - seated<br/>53x13 - seated<br/>53x12 - stand<br/>53x13 - stand<br/>53x14 - stand<br/>53x15 - stand53x16 - stand<br/>53x17 - stand<br/><br/>Each sprint should last 10 - 20 seconds or until you stop accelerating. Recover 5 minutes between sprints.",
            "info": "",
            "target": "Endurance wattage: ([1h]*0.7) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Strength Core",
            "shortname": "strengthcore",
            "type": "strength",
            "wu": "15 minutes cardio.",
            "desc": "Weight routine: Yoga, Pilates, back extensions, etc. Goal: strengthen core.",
            "info": "Rest day for the legs.",
            "target": "",
			"cd": "",
			"duration": "90"
        },
        {
            "name": "Weights: Strength Phase I",
            "shortname": "strength1",
            "type": "strength",
            "wu": "15 minutes cardio.",
            "desc": "Weight routine. The goal of this workout is to increase muscle endurance. Low-weight, high-rep.<ul><li>back extension</li><li>calf raises</li><li>pullups</li><li>goblet squats</li><li>seated rows</li><li>lunge</li><li>flat db flys</li><li>incline db press</li><li>seated military db press</li><li>db curls</li><li>rope tricep pulldown</li><li>situps</li></ul>",
            "info": "This phase begins at the very beginning of base training, after the post-season break. Most exercises should be 3 sets of 30 reps, otherwise, 2 sets of maximum reps.",
            "target": "",
			"cd": "Stretching.",
			"duration": "90"
        },
        {
            "name": "Weights: Strength Phase II",
            "shortname": "strength2",
            "type": "strength",
            "wu": "15 minutes cardio.",
            "desc": "Weight routine: The goal of this workout is to increase muscle strength. Low-rep, high-weight.<ul><li>weighted back extension</li><li>calf raises</li><li>weighted pullups</li><li>squats</li><li>seated rows</li><li>leg press</li><li>flat db flys</li><li>incline db press</li><li>seated military db press</li><li>lateral shoulder raise</li><li>db curls</li><li>rope tricep pulldown</li><li>decline weighted situps</li></ul>",
            "info": "This phase begins in the middle of base training, just before you start working on shorter intervals. Most exercises should be 3 sets of 5 reps, otherwise, 3 sets of maximum reps.",
            "target": "",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Weights: Strength Phase III",
            "shortname": "strength3",
            "type": "strength",
            "wu": "15 minutes cardio.",
            "desc": "Weight routine: mid rep, mid weight. Goal: muscle power.",
            "info": "This phase begins a couple months before racing season begins, to give you that final pop.",
            "target": "",
			"cd": "",
			"duration": "90"
        },
        {
            "name": "Sweet Spot - 2x30 minutes",
            "shortname": "sweetspot-2x30minutes",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Start the ride with 30 minutes at 10%-15% below FTP. Ride in your endurance zone for the majority of the ride. Finish with 30 minutes at 10% - 15% below FTP.",
            "info": "Stay aero and smooth like you are riding a break.",
            "target": "Sweet Spot wattage: ([1h]*.85) watts. In between: ([1h]*.7) watts",
			      "cd": "Easy spinning for 15 minutes. 100+ cadence",
			      "duration": "90"
        },
        {
            "name": "Sweet Spot - 3x20 minutes",
            "shortname": "sweetspot-3x20minutes",
            "type": "bike",
            "wu": "20 - 30 minutes including 1 x 5 minutes of hard pedaling and 10 x 1 minute of high cadence.",
            "desc": "Complete 3 x 20 minutes in 10-15% below FTP. Recover between intervals by riding for 10 minutes in your endurance zone.",
            "info": "Stay aero and smooth like you are riding a break.",
            "target": "Sweet Spot wattage: ([1h]*.85) watts. In between: ([1h]*.7) watts",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Tempo - 1x45 minutes",
            "shortname": "tempo-1x45minutes",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Complete 1 x 45 minutes of riding in your Tempo power zone. During the interval, altenate riding 5 minutes with your cadence below 75 and riding 5 minutes with your candence above 105. Use your gearing to vary your cadence while aiming to maintain the power level in your tempo zone.",
            "info": "",
            "target": "Tempo wattage: ([1h]*.80) watts. In between: ([1h]*.7) watts",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Tempo - 1x60 minutes",
            "shortname": "tempo-1x1hinutes",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Complete a 60 minute interval aiming to keep your average and normalized power in the middle of your tempo power zone. Alternate pedaling for 10 minutes with your cadence between 70 - 75 rpm and 10 minutes with your cadence between 95 and 105. Use your gearing to vary your cadence while aiming to maintain the power level in your tempo zone.",
            "info": "",
            "target": "Tempo wattage: ([1h]*.80) watts. In between: ([1h]*.7) watts",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Tempo - 2x10 minutes",
            "shortname": "tempo-2x10minutes",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Complete 2 x 10 minutes of riding in your Tempo power zone. Do the first interval with your cadence between 70 - 75 rpm. Ride for 5 minutes in your endurance zone. Do the second interval with your cadence between 95 and 105 rpm. Use your gearing to vary your cadence while aiming to maintain the power level in your tempo zone.",
            "info": "If you are stuck inside, warmup on the trainer for 30 minutes and then do the intervals and then cool down for 15 minutes.",
            "target": "Tempo wattage: ([1h]*.80) watts. In between: ([1h]*.7) watts",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Tempo - 2x15 minutes",
            "shortname": "tempo-2x15minutes",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Complete 2 x 15 minutes of riding in your Tempo power zone. Do the first interval with your cadence between 70 - 75 rpm. Ride for 10 minutes in your endurance zone. Do the second interval with your cadence above 105 rpm. Use your gearing to vary your cadence while aiming to maintain the power level in your tempo zone.",
            "info": "",
            "target": "Tempo wattage: ([1h]*.80) watts. In between: ([1h]*.7) watts",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Tempo - 2x30 minutes",
            "shortname": "tempo-2x30minutes",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Complete 2 x 30 minutes of riding in your Tempo power zone. During each interval, alternate riding 5 minutes with your cadence below 75 and riding 5 minutes with your candence above 105. Ride for 15 minutes in your endurance zone between intervals. Use your gearing to vary your cadence while aiming to maintain the power level in your tempo zone.",
            "info": "",
            "target": "Tempo wattage: ([1h]*.80) watts. In between: ([1h]*.7) watts",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Tempo - Overgeared Climbs",
            "shortname": "tempo-overgearedclimbs",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Keep the power in your tempo zone on any climbs, but use a gear that is one click bigger than you are comfortable riding in. Your cadence will drop 10 - 15 rpm, and that is OK.",
            "info": "",
            "target": "Tempo wattage: ([1h]*.80) watts. In between: ([1h]*.7) watts",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Tempo - Endurance with Tempo",
            "shortname": "tempo-endurancewithtempo",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Keep the effort steady through the whole ride.",
            "info": "",
            "target": "Sweet Spot wattage: ([1h]*.85) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Tempo - Climbs",
            "shortname": "tempo-climbs",
            "type": "bike",
            "wu": "Slowly work your way into your endurance zone over the first 15 minutes of the ride. Endurance = cadence above 90, smooth and steady effort.",
            "desc": "Keep the power in your tempo zone on any climbs. Use the gears to flatten out the course.",
            "info": "",
            "target": "Tempo wattage: ([1h]*.80) watts. In between: ([1h]*.7) watts",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Test - Day 1 (long power)",
            "shortname": "test-day1longpower",
            "type": "test",
            "wu": "Warm-up for one hour. You want to have about 400kj in your legs at the start of this test. Include 3 x 1 minute all-out efforts and 3 x 1 minute high cadence riding and 1 x 5 minutes of hard pedaling.",
            "desc": "Before starting, ride easy for 5 minutes. We are going for highest average power during each interval. Go as hard as you can for 20 minutes. Recover for 20 - 30 minutes. Do it again and see if you can get a better result.",
            "info": "",
            "target": "",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Test - Day 1 TRAINER (long power)",
            "shortname": "test-day1trainerlongpower",
            "type": "test",
            "wu": "Warm-up for one hour. You want to have about 400kj in your legs at the start of this test. Include 3 x 1 minute all-out efforts and 3 x 1 minute high cadence riding and 1 x 5 minutes of hard pedaling.",
            "desc": "Before starting, ride easy for 5 minutes. This is a ramp test to failure. See the Target Power information to perform the test correctly.",
            "info": "",
            "target": "Start with 3 minutes at ([1h]*0.599245903162305). Increase the power by 20 watts every 3 minutes. When you get up to ([1h]*0.899811475790576), increase the power by 10 watts every 3 minutes. Continue increasing power by 10 watts every 3 minutes until you cannot complete 3 minutes at a level.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Test - Day 2 (short power)",
            "shortname": "test-day2shortpower",
            "type": "test",
            "wu": "Warm-up for one hour. You want to have about 400kj in your legs at the start of this test. Include 3 x 1 minute all-out efforts and 3 x 1 minute high cadence riding and 1 x 5 minutes of hard pedaling.",
            "desc": "Before starting, ride easy for 5 minutes. We are going for highest average power during each interval.<br/>Go as hard as you can for 5 minutes. Really hammer and give it a last gun in the last 45 seconds.<br/> 10 minutes easy spinning<br/> 2 minutes all-out. Really nail it for the entire 2 minutes like you are going for the gold.<br/> 5 minutes easy spinning<br/> 2 minutes all-out.<br/> 5 minutes easy spinning<br/> 1 minute all-out. Really hit these hard for the whole minute.<br/> 5 minutes easy spinning<br/> 1 minute all-out.<br/> 5 minutes easy spinning<br/> 30 seconds all-out.<br/> 5 minutes easy spinning.<br/> 30 seconds all-out.<br/> 2 minutes easy spinning.<br/> 12-second out of the saddle sprint.<br/> 2 minutes easy spinning.<br/> One last 12-second sprint. Give it all you got.",
            "info": "",
            "target": "",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "Test - Day 2 (peak power)",
            "shortname": "test-day2peakpower",
            "type": "test",
            "wu": "Get in a good warm-up by riding for 20 - 30 minutes working your way up to your endurance zone over the first 15 minutes and then including a 5 minutes ramp up to FTP increasing the watts 10 - 20 every minute. Cruise in your endurance zone for 5 minutes before starting the test.",
            "desc": "Before starting, ride easy for 5 minutes. This is a ramp test to failure. See the Target Power information to perform the test correctly.",
            "info": "",
            "target": "Start with 3 minutes at ([1h]*0.599245903162305). Increase the power by 20 watts every 3 minutes. When you get up to ([1h]*0.899811475790576), increase the power by 10 watts every 3 minutes. Continue increasing power by 10 watts every 3 minutes until you cannot complete 3 minutes at a level.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "VO2 - 3 minutes",
            "shortname": "vo2-3minutes",
            "type": "bike",
            "wu": "5 x 1 minute of high cadence pedaling and 1 x 5 minutes of high threshold effort.",
            "desc": "Complete 7 x 3 minutes in your VO2 zone with 3 minutes of recovery in between each effort. These are hard. Go as hard as you can and still finish the 3 minutes each time.",
            "info": "",
            "target": "([5m]*1) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "VO2 - 3 minutes (hills)",
            "shortname": "vo2-3minuteshills",
            "type": "bike",
            "wu": "5 x 1 minute of high cadence pedaling and 1 x 5 minutes of high threshold effort.",
            "desc": "Find a hill that takes 2 - 4 minutes to climb at a steady but hard pace. Recover for the same amount of time as the preceding interval. Complete 7 - 10 intervals. Once you are unable to maintain the target power for 2m30s, head home.",
            "info": "Find a hill that you can do repeats on. It does not have to be super steep, just a steady grade. If you cant find a hill, then do them on the flats but get that power up to target and hold it.",
            "target": "([5m]*1) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "VO2 - 3 minutes (speed)",
            "shortname": "vo2-3minutesspeed",
            "type": "bike",
            "wu": "5 x 1 minute of high cadence pedaling and 1 x 5 minutes of high threshold effort.",
            "desc": "Find an open section of road that is mostly flat where you can go hard and fast without interuptions from lights or intersections or trafific. Each interval is 3 - 5 minutes. Recovery between intervals is for the same amount of time as the preceding interval. Complete 7 - 10 intervals. Once you are unable to maintain the target power for at least 2m30s, head home.",
            "info": "",
            "target": "([5m]*1) watts.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "VO2 - 30 seconds/30 seconds",
            "shortname": "vo2-30secondsx30seconds",
            "type": "bike",
            "wu": "5 x 1 minute of high cadence pedaling and 1 x 5 minutes of high threshold effort.",
            "desc": "Start with 3 x 3 minutes on, 3 minutes off working from high threshold to your VO2max zone during the on intervals and mid endurance during the off intervals. Ride 5 minutes in your endurance zone before starting the next bit. This workout is basically 30 seconds at VO2max followed by 30 seconds at 70% of VO2max. Do 3 sets sets of 10 x 30 seconds on, 30 seconds off. 5 minutes in your endurance zone between sets.",
            "info": "Treat the initial 3 x 3 as part of your warm up.",
            "target": "First 3: ([5m]*0.700525394045534) watts, ([5m]*0.74430823117338) watts, ([5m]*0.788091068301226) watts. For each 30 second interval, target ([5m]*1) watts for ON, ([5m]*0.70) for OFF.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "VO2 - 40 seconds/20 seconds",
            "shortname": "vo2-40secondsx20seconds",
            "type": "bike",
            "wu": "5 x 1 minute of high cadence pedaling and 1 x 5 minutes of high threshold effort.",
            "desc": "Start with 3 x 3 minutes on, 3 minutes off working from high threshold to your VO2max zone during the on intervals and mid endurance during the off intervals. Ride 5 minutes in your endurance zone before starting the next bit. This workout is basically 40 seconds at VO2max followed by 20 seconds at 70% of VO2max. Do 3 sets sets of 10 x 40 seconds on, 20 seconds off. 5 minutes in your endurance zone between sets.",
            "info": "",
            "target": "First 3: ([5m]*0.700525394045534) watts, ([5m]*0.74430823117338) watts, ([5m]*0.788091068301226) watts. For each 30 second interval, target ([5m]*1) watts for ON, ([5m]*0.70) for OFF.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "VO2 - 40 seconds/30 seconds",
            "shortname": "vo2-40secondsx30seconds",
            "type": "bike",
            "wu": "5 x 1 minute of high cadence pedaling and 1 x 5 minutes of high threshold effort.",
            "desc": "Start with 3 x 3 minutes on, 3 minutes off working from high threshold to your VO2max zone during the on intervals and mid endurance during the off intervals. Ride 5 minutes in your endurance zone before starting the next bit. This workout is basically 40 seconds at VO2max followed by 30 seconds at 70% of VO2max. Do 3 sets sets of 10 x 40 seconds on, 30 seconds off. 5 minutes in your endurance zone between sets.",
            "info": "",
            "target": "First 3: ([5m]*0.700525394045534) watts, ([5m]*0.74430823117338) watts, ([5m]*0.788091068301226) watts. For each 30 second interval, target ([5m]*1) watts for ON, ([5m]*0.70) for OFF.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        },
        {
            "name": "VO2 - Ladder 1-4 minutes",
            "shortname": "vo2-ladder1-4minutes",
            "type": "bike",
            "wu": "5 x 1 minute of high cadence pedaling and 1 x 5 minutes of high threshold effort.",
            "desc": "Cruise for 5 minutes before starting these intervals. Complete the following sequence: 1 minute on, 1 minute off. 2 minutes on, 2 minutes off. 3 minutes on, 3 minutes off. 4 minutes on, 4 minutes off. 4 minutes on, 4 minutes off. 3 minutes on, 3 minutes off. 2 minutes on, 2 minutes off. 1 minute on, 1 minute off. 1 minute on, 1 minute off. 1 minute on, 1 minute off. 1 minute on, 1 minute off. 1 minute on, 1 minute off.",
            "info": "",
            "target": "([5m]*1) watts for ON, ([1h]*0.60) for OFF.",
			"cd": "Easy spinning for 15 minutes. 100+ cadence",
			"duration": "90"
        }
    ],
    "schedules": [
        {
            "name": "43-09-tempo1",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "tempo-2x10minutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-2x10minutes"
            }
        },
        {
            "name": "44-09-tempo2",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "tempo-2x15minutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-2x15minutes"
            }
        },
        {
            "name": "45-09-tempo3",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "tempo-2x30minutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-2x30minutes"
            }
        },
        {
            "name": "46-09-test1",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
        {
            "name": "47-09-test2",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "peak-vo2acstepups"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "test-day2shortpower"
            }
        },
        {
            "name": "48-09-tempo4",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "tempo-1x45minutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-1x1hinutes"
            }
        },
        {
            "name": "49-09-sweetspot1",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "50-09-test1",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
        {
            "name": "51-09-ftp1",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "ftp-3x20minutes"
            },
            "thu": {
                "workoutname": "strength2"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ftp-3x20minutes"
            }
        },
        {
            "name": "52-09-ftp2",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "ftp-3x20minutes"
            },
            "thu": {
                "workoutname": "strength2"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ftp-2x20minutes"
            }
        },
        {
            "name": "01-10-ftp3",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "ftp-2x20minutes"
            },
            "thu": {
                "workoutname": "strength2"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ftp-2x20minutes"
            }
        },
        {
            "name": "02-10-test1",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
        {
            "name": "03-10-vo21",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "vo2-3minutes"
            },
            "thu": {
                "workoutname": "strength2"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "04-10-vo22",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "vo2-3minutes"
            },
            "thu": {
                "workoutname": "strength2"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "05-10-vo23",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "vo2-3minutes"
            },
            "thu": {
                "workoutname": "strength2"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "06-10-test2",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "test-day2shortpower"
            }
        },
        {
            "name": "07-10-vo21",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-40secondsx30seconds"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "strength3"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minutes"
            }
        },
        {
            "name": "08-10-vo22",
            "sun": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-40secondsx30seconds"
            },
            "wed": {
                "workoutname": "ftp-5x3minutes"
            },
            "thu": {
                "workoutname": "strength3"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minutes"
            }
        },
        {
            "name": "09-10-vo23",
            "sun": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-40secondsx30seconds"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "strength3"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "10-10-test2",
            "sun": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "test-day2shortpower"
            }
        },
        {
            "name": "11-10-bursts1",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "bursts-basicbursts"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "vo2-40secondsx30seconds"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-climbs"
            }
        },
        {
            "name": "12-10-bursts2",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "bursts-basicbursts"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "vo2-40secondsx30seconds"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-climbs"
            }
        },
        {
            "name": "13-10-sprints3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "sprints-9"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sprints-9"
            }
        },
        {
            "name": "14-10-sprints4",
            "sun": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "sprints-9"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sprints-9"
            }
        },
        {
            "name": "15-10-sweetspot5",
            "sun": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "sprints-sprintstarts"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "16-10-ac6",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "ftp-2x10minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ac-30seconds"
            }
        },
        {
            "name": "17-10-maint7",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ftp-2x10minutes"
            },
            "wed": {
                "workoutname": "sprints-sprintstarts"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "18-10-maint8",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30seconds"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "19-10-easy9",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "20-10-maint10",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-powerwindups"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-9"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "21-10-maint11",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-powerwindups"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-9"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "raceprep"
            }
        },
        {
            "name": "22-10-maint12",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-powerwindups"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "23-10-maint13",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ac-racewinners"
            }
        },
        {
            "name": "24-10-maint14",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-3minutes"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "ftp-3x10minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ac-racewinners"
            }
        },
        {
            "name": "25-10-maint16",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-3minutes"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "26-10-maint17",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-3minutes"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "ftp-3x10minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "raceprep"
            }
        },
        {
            "name": "27-10-maint18",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ac-racewinners"
            }
        },
        {
            "name": "28-10-maint19",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "bursts-microbursts"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "29-10-maint20",
            "sun": {
                "workoutname": "strengthcore"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "30-10-maint21",
            "sun": {
                "workoutname": "strengthcore"
            },
            "mon": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "bursts-microbursts"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "31-10-endurance22",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "32-10-maint23",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-3minuteshills"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-9"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "33-10-maint24",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-powerwindups"
            },
            "wed": {
                "workoutname": "sprints-sprintstarts"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "34-10-maint25",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-3minuteshills"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-9"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "raceprep"
            }
        },
        {
            "name": "35-10-endurance26",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "36-10-off27",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "37-10-off28",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "38-10-off29",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "39-10-off30",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
        {
            "name": "40-10-tempo1",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "tempo-2x15minutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-2x15minutes"
            }
        },
        {
            "name": "41-10-tempo2",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "tempo-2x30minutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-2x30minutes"
            }
        },
        {
            "name": "42-10-tempo3",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "tempo-1x45minutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-1x1hinutes"
            }
        },
        {
            "name": "43-10-test1",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
        {
            "name": "44-10-ftp1",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "ftp-2x20minutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ftp-2x20minutes"
            }
        },
        {
            "name": "45-10-ftp2",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "ftp-2x20minutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ftp-2x20minutes"
            }
        },
        {
            "name": "46-10-ftp3",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "ftp-2x20minutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ftp-2x20minutes"
            }
        },
        {
            "name": "47-10-test1",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
        {
            "name": "48-10-ftp1",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "ftp-2x20minutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-overgearedclimbs"
            }
        },
        {
            "name": "49-10-ftp2",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "ftp-2x20minutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-overgearedclimbs"
            }
        },
        {
            "name": "50-10-ftp3",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "ftp-2x20minutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-overgearedclimbs"
            }
        },
        {
            "name": "51-10-test1",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
        {
            "name": "52-10-off1",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "01-11-off2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "02-11-endurance1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "endurance-basic"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "03-11-ftp1",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "ftp-3x10minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
        {
            "name": "04-11-test1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "vo2-ladder1-4minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
        {
            "name": "05-11-test2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "vo2-3minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "test-day2shortpower"
            }
        },
        {
            "name": "06-11-endurance1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "07-11-ac1",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "ac-30seconds"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "08-11-ac2",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "ac-30-60seconds"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "09-11-ac3",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "ac-30seconds"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "10-11-test2",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "bursts-basicbursts"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "test-day2shortpower"
            }
        },
        {
            "name": "11-11-ac1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-45seconds"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ac-1-2minuteshills"
            }
        },
        {
            "name": "12-11-ac2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "ac-45seconds"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "13-11-ac3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30-60seconds"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "raceprep"
            }
        },
        {
            "name": "14-11-sprints1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "strengthcore"
            },
            "wed": {
                "workoutname": "sprints-sprintstarts"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "15-11-sprints2",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30-60seconds"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-9"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sprints-stompsprintsstanding"
            }
        },
        {
            "name": "16-11-sprints3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-3x30seconds"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-6"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "17-11-test2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-3x30seconds"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-6"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "test-day2shortpower"
            }
        },
        {
            "name": "18-11-endurance1",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "sprints-sprintstarts"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "raceprep"
            }
        },
        {
            "name": "19-11-vo21",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-3minuteshills"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-6"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "20-11-vo22",
            "sun": {
                "workoutname": "tempo-endurancewithtempo"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "wed": {
                "workoutname": "vo2-3minuteshills"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "21-11-vo23",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-3minuteshills"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-9"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "22-11-endurance1",
            "sun": {
                "workoutname": "tempo-climbs"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "23-11-vo21",
            "sun": {
                "workoutname": "tempo-climbs"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-1ktimetrial"
            },
            "wed": {
                "workoutname": "tempo-endurancewithtempo"
            },
            "thu": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "24-11-vo22",
            "sun": {
                "workoutname": "bigblock-vo2"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "tempo-endurancewithtempo"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "ac-1ktimetrial"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ftp-3x15minutes"
            }
        },
        {
            "name": "25-11-vo23",
            "sun": {
                "workoutname": "tempo-endurancewithtempo"
            },
            "mon": {
                "workoutname": "vo2-3minuteshills"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "sprints-6"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "26-11-maint4",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "bursts-basicbursts"
            },
            "wed": {
                "workoutname": "ac-1ktimetrial"
            },
            "thu": {
                "workoutname": "sprints-6"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "raceprep"
            }
        },
        {
            "name": "27-11-maint5",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "ac-racewinners"
            },
            "wed": {
                "workoutname": "tempo-endurancewithtempo"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ac-racewinners"
            }
        },
        {
            "name": "28-11-endurance1",
            "sun": {
                "workoutname": "endurance-basicgroup"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "endurance-basic+attacks"
            }
        },
        {
            "name": "29-11-maint1",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "ftp-3x10minutes"
            },
            "wed": {
                "workoutname": "tempo-endurancewithtempo"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "bigblock-vo2"
            }
        },
        {
            "name": "30-11-maint2",
            "sun": {
                "workoutname": "bigblock-sprints"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "ftp-3x10minutes"
            },
            "wed": {
                "workoutname": "tempo-endurancewithtempo"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "31-11-maint3",
            "sun": {
                "workoutname": "tempo-endurancewithtempo"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "32-11-maint4",
            "sun": {
                "workoutname": "bigblock-vo2"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "bigblock-sprints"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "bigblock-attacks"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "raceprep"
            }
        },
        {
            "name": "33-11-maint5",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "ac-1ktimetrial"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-9"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "34-11-maint6",
            "sun": {
                "workoutname": "endurance-basic+attacks"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "ac-1ktimetrial"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-9"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "raceprep"
            }
        },
        {
            "name": "35-11-off1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "36-11-off2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "37-11-off3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "38-11-off4",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "39-11-off5",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "40-11-off6",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "41-11-off7",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "42-11-tempo1",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x10minutes"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "tempo-2x10minutes"
            }
        },
        {
            "name": "43-11-tempo2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x15minutes"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "tempo-2x15minutes"
            }
        },
        {
            "name": "44-11-tempo3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x30minutes"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "tempo-2x30minutes"
            }
        },
        {
            "name": "45-11-tempo4",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-1x45minutes"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "tempo-1x1hinutes"
            }
        },
        {
            "name": "46-11-endurance1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "47-11-test1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "tempo-1x1hinutes"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
        {
            "name": "48-11-tempo1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "tempo-endurancewithtempo"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "49-11-tempo2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "tempo-endurancewithtempo"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "50-11-endurance1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "racepreptt"
            }
        },
        {
            "name": "51-11-tempo1",
            "sun": {
                "workoutname": "test-day1longpower"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "wed": {
                "workoutname": "strength2"
            },
            "thu": {
                "workoutname": "tempo-endurancewithtempo"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "52-11-tempo2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "wed": {
                "workoutname": "strength2"
            },
            "thu": {
                "workoutname": "tempo-endurancewithtempo"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "53-11-tempo3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "wed": {
                "workoutname": "strength2"
            },
            "thu": {
                "workoutname": "tempo-endurancewithtempo"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "01-12-endurance1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "racepreptt"
            }
        },
        {
            "name": "02-12-test2",
            "sun": {
                "workoutname": "test-day1longpower"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "peak-vo2acstepups"
            },
            "wed": {
                "workoutname": "strength2"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "test-day2shortpower"
            }
        },
        {
            "name": "03-12-vo21",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "vo2-3minutes"
            },
            "wed": {
                "workoutname": "strength2"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "04-12-vo22",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "vo2-3minutes"
            },
            "wed": {
                "workoutname": "strength2"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "05-12-endurance3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "endurance-groupride"
            }
        },
        {
            "name": "06-12-vo24",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "wed": {
                "workoutname": "strength3"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "07-12-vo25",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "wed": {
                "workoutname": "strength3"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "08-12-vo26",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "wed": {
                "workoutname": "strength3"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "09-12-test2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "strengthcore"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "test-day2shortpower"
            }
        },
        {
            "name": "10-12-ac1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "ac-30seconds"
            }
        },
        {
            "name": "11-12-ac2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "ac-30seconds"
            }
        },
        {
            "name": "12-12-ac3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "ac-30seconds"
            }
        },
        {
            "name": "13-12-test2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "strengthcore"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "test-day2shortpower"
            }
        },
        {
            "name": "14-12-tempo1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "bursts-microbursts"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "15-12-tempo2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "bursts-microbursts"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "16-12-tempo3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "bursts-microbursts"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "17-12-test1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "tempo-2x30minutes"
            },
            "wed": {
                "workoutname": "sprints-sprintstarts"
            },
            "thu": {
                "workoutname": "strengthcore"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
        {
            "name": "18-12-vo21",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-stompsprintsseated"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "vo2-3minutesspeed"
            }
        },
        {
            "name": "19-12-vo22",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-40secondsx30seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-stompsprintsseated"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "20-12-maint1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-40secondsx20seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-stompsprintsseated"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "21-12-maint2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "22-12-maint3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "23-12-maint4",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "ftp-3x10minutes"
            },
            "wed": {
                "workoutname": "sprints-sprintstarts"
            },
            "thu": {
                "workoutname": "endurance-groupride"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "bursts-basicbursts"
            }
        },
        {
            "name": "24-12-maint5",
            "sun": {
                "workoutname": "bursts-basicbursts"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "bursts-microbursts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "25-12-maint6",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "bursts-microbursts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "26-12-maint7",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "27-12-maint8",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "sprints-9"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "raceprep"
            }
        },
        {
            "name": "28-12-maint9",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "endurance-basic"
            },
            "tue": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "custo"
            }
        },
        {
            "name": "29-12-maint10",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sprints-9"
            }
        },
        {
            "name": "30-12-test2",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "test-day2shortpower"
            }
        },
        {
            "name": "31-12-maint1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sprints-9"
            },
            "wed": {
                "workoutname": "race"
            },
            "thu": {
                "workoutname": "bursts-microbursts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "32-12-maint2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sprints-9"
            },
            "wed": {
                "workoutname": "race"
            },
            "thu": {
                "workoutname": "bursts-microbursts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "33-12-maint3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sprints-6"
            },
            "wed": {
                "workoutname": "race"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "raceprep"
            }
        },
        {
            "name": "34-12-maint4",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sprints-9"
            },
            "wed": {
                "workoutname": "race"
            },
            "thu": {
                "workoutname": "bursts-microbursts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
         {
            "name": "35-12-maint5",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "run-easy"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
         {
            "name": "36-12-endurance1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "run-easy"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
         {
            "name": "37-12-endurance2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "run-easy"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
         {
            "name": "38-12-endurance3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "run-easy"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
         {
            "name": "39-12-test1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x10minutes"
            },
            "wed": {
                "workoutname": "tempo-2x10minutes"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
         {
            "name": "40-12-tempo1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x10minutes"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "tempo-2x10minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-2x10minutes"
            }
        },
         {
            "name": "41-12-tempo2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x15minutes"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "tempo-2x15minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-2x15minutes"
            }
        },
        {
            "name": "42-12-tempo3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x30minutes"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "tempo-2x30minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-2x30minutes"
            }
        },
        {
            "name": "43-12-test1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },

        {
            "name": "44-12-tempo1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x30minutes"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "tempo-2x30minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-2x30minutes"
            }
        },
        {
            "name": "45-12-tempo2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-1x45minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-1x45minutes"
            }
        },
        {
            "name": "46-12-tempo3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x30minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-1x1hinutes"
            }
        },
        {
            "name": "47-12-test1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
       {
            "name": "48-12-ftp1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "test-day1trainerlongpower"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "strength2"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ftp-3x20minutes"
            }
        },
       {
            "name": "49-12-ftp2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "ftp-2x20minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "strength2"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ftp-3x20minutes"
            }
        },
       {
            "name": "50-12-ftp3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "ftp-2x20minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "strength2"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ftp-3x20minutes"
            }
        },
       {
            "name": "51-12-test1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength2"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
       {
            "name": "52-12-test2",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "peak-vo2acstepups"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "test-day2shortpower"
            }
        },
       {
            "name": "01-13-ftp1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "ftp-3x20minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "ftp-3x20minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ac-racewinners"
            }
        },
       {
            "name": "02-13-ftp2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "ftp-3x20minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "ftp-3x20minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ac-racewinners"
            }
        },
       {
            "name": "03-13-ftp3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "bursts-2x20minutes"
            },
            "wed": {
                "workoutname": "bursts-2x20minutes"
            },
            "thu": {
                "workoutname": "bursts-2x20minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ac-racewinners"
            }
        },
        {
            "name": "04-13-test1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
        {
            "name": "05-13-testpeak",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "raceprep"
            },
            "wed": {
                "workoutname": "test-day2peakpower"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ac-racewinners"
            }
        },
        {
            "name": "06-13-vo21",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "vo2-3minutes"
            },
            "wed": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "thu": {
                "workoutname": "vo2-3minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "07-13-vo22",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "wed": {
                "workoutname": "vo2-3minutes"
            },
            "thu": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "08-13-vo23",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "vo2-3minutes"
            },
            "wed": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "thu": {
                "workoutname": "vo2-3minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "09-13-vo24",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "vo2-ladder1-4minutes"
            },
            "wed": {
                "workoutname": "vo2-3minutes"
            },
            "thu": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "10-13-endurance",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "11-13-ac1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "ac-30seconds"
            }
        },
        {
            "name": "12-13-ac2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "ac-30seconds"
            }
        },
        {
            "name": "13-13-ac3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "ac-30seconds"
            }
        },
        {
            "name": "14-13-sprints1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "strengthcore"
            },
            "wed": {
                "workoutname": "sprints-sprintstarts"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "15-13-sprints2",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30-60seconds"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-9"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sprints-stompsprintsstanding"
            }
        },
        {
            "name": "16-13-sprints3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-3x30seconds"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-6"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "17-13-endurance1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "run-easy"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "18-13-vo21",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "vo2-3minutes"
            },
            "wed": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "thu": {
                "workoutname": "vo2-3minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "19-13-vo22",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "wed": {
                "workoutname": "vo2-3minutes"
            },
            "thu": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "20-13-vo23",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "vo2-3minutes"
            },
            "wed": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "thu": {
                "workoutname": "vo2-3minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "21-13-maint1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-40secondsx20seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-stompsprintsseated"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "22-13-maint2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "23-13-maint3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "24-13-maint4",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "ftp-3x10minutes"
            },
            "wed": {
                "workoutname": "sprints-sprintstarts"
            },
            "thu": {
                "workoutname": "endurance-groupride"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "bursts-basicbursts"
            }
        },
        {
            "name": "25-13-maint5",
            "sun": {
                "workoutname": "bursts-basicbursts"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "bursts-microbursts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "26-13-maint6",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "bursts-microbursts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "27-13-maint7",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "28-13-maint8",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "sprints-9"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "raceprep"
            }
        },
        {
            "name": "29-13-maint9",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "endurance-basic"
            },
            "tue": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "custo"
            }
        },
        {
            "name": "30-13-maint10",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sprints-9"
            }
        },
        {
            "name": "31-13-test2",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "test-day2shortpower"
            }
        },
        {
            "name": "32-13-maint2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sprints-9"
            },
            "wed": {
                "workoutname": "race"
            },
            "thu": {
                "workoutname": "bursts-microbursts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "33-13-maint3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sprints-6"
            },
            "wed": {
                "workoutname": "race"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "raceprep"
            }
        },
        {
            "name": "34-13-maint4",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sprints-9"
            },
            "wed": {
                "workoutname": "race"
            },
            "thu": {
                "workoutname": "bursts-microbursts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
         {
            "name": "35-13-maint5",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "run-easy"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
         {
            "name": "36-13-endurance1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "run-easy"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
         {
            "name": "37-13-endurance2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "run-easy"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
         {
            "name": "38-13-endurance3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "run-easy"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
         {
            "name": "39-13-test1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x10minutes"
            },
            "wed": {
                "workoutname": "tempo-2x10minutes"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
         {
            "name": "40-13-tempo1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x10minutes"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "tempo-2x10minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-2x10minutes"
            }
        },
         {
            "name": "41-13-tempo2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x15minutes"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "tempo-2x15minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-2x15minutes"
            }
        },
        {
            "name": "42-13-tempo3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x30minutes"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "tempo-2x30minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-2x30minutes"
            }
        },
        {
            "name": "43-13-test1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "racepreptt"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },

        {
            "name": "44-13-tempo1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x30minutes"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "tempo-2x30minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-2x30minutes"
            }
        },
        {
            "name": "45-13-tempo2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-1x45minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-1x45minutes"
            }
        },
        {
            "name": "46-13-tempo3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "tempo-2x30minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "tempo-1x1hinutes"
            }
        },
        {
            "name": "47-13-test1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
       {
            "name": "48-13-ftp1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "test-day1trainerlongpower"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "strength2"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ftp-3x20minutes"
            }
        },
       {
            "name": "49-13-ftp2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "ftp-2x20minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "strength2"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ftp-3x20minutes"
            }
        },
       {
            "name": "50-13-ftp3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "ftp-2x20minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "strength2"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ftp-3x20minutes"
            }
        },
       {
            "name": "51-13-test1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength2"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "test-day1longpower"
            }
        },
       {
            "name": "52-13-test2",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "easy-off"
            },
            "wed": {
                "workoutname": "peak-vo2acstepups"
            },
            "thu": {
                "workoutname": "strength1"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "test-day2shortpower"
            }
        },
        {
            "name": "01-14-ftp1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "ftp-3x20minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "ftp-3x20minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ac-racewinners"
            }
        },
       {
            "name": "02-14-ftp2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "ftp-3x20minutes"
            },
            "wed": {
                "workoutname": "bursts-basicbursts"
            },
            "thu": {
                "workoutname": "ftp-3x20minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ac-racewinners"
            }
        },
       {
            "name": "03-14-ftp3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength2"
            },
            "tue": {
                "workoutname": "bursts-2x20minutes"
            },
            "wed": {
                "workoutname": "bursts-2x20minutes"
            },
            "thu": {
                "workoutname": "bursts-2x20minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ac-racewinners"
            }
        },
        {
            "name": "04-14-testpeak",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "raceprep"
            },
            "wed": {
                "workoutname": "test-day2peakpower"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "ac-racewinners"
            }
        },
        {
            "name": "05-14-vo21",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "vo2-3minutes"
            },
            "wed": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "thu": {
                "workoutname": "vo2-3minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "06-14-vo22",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "wed": {
                "workoutname": "vo2-3minutes"
            },
            "thu": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "07-14-vo23",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "vo2-3minutes"
            },
            "wed": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "thu": {
                "workoutname": "vo2-3minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "08-14-vo24",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "vo2-ladder1-4minutes"
            },
            "wed": {
                "workoutname": "vo2-3minutes"
            },
            "thu": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "09-14-endurance",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "10-14-ac1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "ac-30seconds"
            }
        },
        {
            "name": "11-14-ac2",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "ac-30seconds"
            }
        },
        {
            "name": "12-14-ac3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30seconds"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-stompsprintsstanding"
            },
            "fri": {
                "workoutname": "strengthcore"
            },
            "sat": {
                "workoutname": "ac-30seconds"
            }
        },
        {
            "name": "13-14-sprints1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "strengthcore"
            },
            "wed": {
                "workoutname": "sprints-sprintstarts"
            },
            "thu": {
                "workoutname": "endurance-basic"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "14-14-sprints2",
            "sun": {
                "workoutname": "endurance-basic"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-30-60seconds"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-9"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sprints-stompsprintsstanding"
            }
        },
        {
            "name": "15-14-sprints3",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "ac-3x30seconds"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-6"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "16-14-endurance1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength1"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "strength1"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "run-easy"
            },
            "sat": {
                "workoutname": "endurance-basic"
            }
        },
        {
            "name": "17-14-vo21",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "vo2-3minutes"
            },
            "wed": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "thu": {
                "workoutname": "vo2-3minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "18-14-vo22",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "wed": {
                "workoutname": "vo2-3minutes"
            },
            "thu": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "19-14-vo23",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strength3"
            },
            "tue": {
                "workoutname": "vo2-3minutes"
            },
            "wed": {
                "workoutname": "vo2-30secondsx30seconds"
            },
            "thu": {
                "workoutname": "vo2-3minutes"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "vo2-3minuteshills"
            }
        },
        {
            "name": "20-14-vo23",
            "sun": {
                "workoutname": "tempo-endurancewithtempo"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "wed": {
                "workoutname": "vo2-3minuteshills"
            },
            "thu": {
                "workoutname": "easy-off"
            },
            "fri": {
                "workoutname": "raceprep"
            },
            "sat": {
                "workoutname": "easy-off"
            }
        },
        {
            "name": "21-14-vo24",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "vo2-3minuteshills"
            },
            "wed": {
                "workoutname": "sweetspot-2x30minutes"
            },
            "thu": {
                "workoutname": "sprints-9"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "22-14-endurance1",
            "sun": {
                "workoutname": "tempo-climbs"
            },
            "mon": {
                "workoutname": "strengthcore"
            },
            "tue": {
                "workoutname": "endurance-basic"
            },
            "wed": {
                "workoutname": "endurance-basic"
            },
            "thu": {
                "workoutname": "sprints-sprintstarts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        },
        {
            "name": "23-14-maint1",
            "sun": {
                "workoutname": "endurance-groupride"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "ftp-3x10minutes"
            },
            "wed": {
                "workoutname": "sprints-sprintstarts"
            },
            "thu": {
                "workoutname": "endurance-groupride"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "bursts-basicbursts"
            }
        },
        {
            "name": "24-14-maint2",
            "sun": {
                "workoutname": "bursts-basicbursts"
            },
            "mon": {
                "workoutname": "easy-off"
            },
            "tue": {
                "workoutname": "sweetspot-3x20minutes"
            },
            "wed": {
                "workoutname": "easy-off"
            },
            "thu": {
                "workoutname": "bursts-microbursts"
            },
            "fri": {
                "workoutname": "easy-off"
            },
            "sat": {
                "workoutname": "sweetspot-2x30minutes"
            }
        }
    ]
}
